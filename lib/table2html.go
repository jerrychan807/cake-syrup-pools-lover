...
func GenerateSyrupPoolTableHtml(SyrupPools *SyrupPools) {
	...
	*/
	...
	// 设置表头
	title := table.Tag("tr")
	title.Tag("th").Text("SyrupPool ID").Bgcolor("#FFFF77")
	title.Tag("th").Text("Logo").Bgcolor("#FFFF77")
	title.Tag("th").Text("Token Name/Symbol").Bgcolor("#FFFF77")
	title.Tag("th").Text("Token Price(USD)").Bgcolor("#FFFF77")
	title.Tag("th").Text("Token PriceUSDChange24h").Bgcolor("#FFFF77")
	//title.Tag("th").Text("RewardToken ContractAddr")
	title.Tag("th").Text("Daily/Weekly/Monthly/Yearly Profit(USD)(Per100Cake)").Bgcolor("#FFFF77")
	title.Tag("th").Text("SyrupPool TotalStaked Cake").Bgcolor("#FFFF77")
	title.Tag("th").Text("SyrupPool StartTime").Bgcolor("#FFFF77").Width("170")
	title.Tag("th").Text("SyrupPool EndTime").Bgcolor("#FFFF77").Width("170")
	title.Tag("th").Text("Txns24h").Bgcolor("#FFFF77")
	//title.Tag("th").Text("Token Txns24hChange")
	title.Tag("th").Text("Volume24hUSD").Bgcolor("#FFFF77")
	//title.Tag("th").Text("Token VolumeUSDChange24h")
	title.Tag("th").Text("LiquidityUSD").Bgcolor("#FFFF77")
	//title.Tag("th").Text("Token LiquidityUSDChange24h")
	title.Tag("th").Text("Fully Diluted Market Cap").Bgcolor("#FFFF77")
	...
	for index, _ := range SyrupPools.SyPools {
		...
		tr.Tag("th").Img().Src(sPool.Token.LogoURI).Width("50").Height("50")
		...
		tr.Tag("th").Text(fmt.Sprintf("%s", util.Float64ToPercentage(sPool.Token.PriceUSDChange24h))).Bgcolor(util.GetColorByChange(sPool.Token.PriceUSDChange24h))
		...
		tr.Tag("th").Text(fmt.Sprintf("%s", sPool.StartTime)).Width("170")
		tr.Tag("th").Text(fmt.Sprintf("%s", sPool.EndTime)).Width("170")
		tr.Tag("th").Text(fmt.Sprintf("%s/%s", strconv.Itoa(sPool.Token.Txns24h), util.Float64ToPercentage(sPool.Token.Txns24hChange)))
		//tr.Tag("th").Text(fmt.Sprintf("%s", util.Float64ToPercentage(sPool.Token.Txns24hChange)))
		tr.Tag("th").Text(fmt.Sprintf("%s/%s", util.Float64ToStr(sPool.Token.Volume24hUSD, 0), util.Float64ToPercentage(sPool.Token.VolumeUSDChange24h)))
		//tr.Tag("th").Text(fmt.Sprintf("%s", util.Float64ToPercentage(sPool.Token.VolumeUSDChange24h)))
		tr.Tag("th").Text(fmt.Sprintf("%s/%s", util.Float64ToStr(sPool.Token.LiquidityUSD, 0), util.Float64ToPercentage(sPool.Token.LiquidityUSDChange24h)))
